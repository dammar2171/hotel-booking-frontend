...
type RoomAction =
  ...
  | {
      type: 'FETCH_SUCCESS';
      payload: { rooms: Room[]; pagination: Pagination };
    }
  | { type: 'FETCH_ERROR'; payload: string }
  | { type: 'SET_SELECTED'; payload: Room }
...
export function RoomProvider({ children }: { children: React.ReactNode }) {
  ...
  const createRoom = useCallback(
    async (data: Omit<Room, 'id'>): Promise<boolean> => {
      try {
  ...
  const updateRoom = useCallback(
    async (id: number, data: Partial<Room>): Promise<boolean> => {
      try {
        ...
        dispatch({
          type: 'FETCH_ERROR',
          payload: error?.response?.message ?? 'Failed to update room.',
        });
        return false;
      }
    },
  ...
  const deleteRoom = useCallback(async (id: number) => {
    try {
      ...
      dispatch({
        type: 'FETCH_ERROR',
        payload: error?.response?.message ?? 'Failed to delete room.',
      });
      return false;
    }
  }, []);
